...
type userProps = {userId: string, name: string, bio: string, 
    language: [string], address: string}
...
export const updateUser = ({userId, name, bio, 
    language, address}:userProps, dispatch: any) => {
    
        return axios({
            method:"put",
            url: `${process.env.REACT_APP_API_URL}api/user` + userId,
            data: {name, bio, language, address}
        })
            .then(() => {
                dispatch({type: UPDATE_USER, payload: name, bio, language})
            })
